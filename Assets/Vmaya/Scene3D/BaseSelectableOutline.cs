...
namespace Vmaya.Scene3D
{
    public class BaseSelectableOutline : SelectableMouse
    {
        ...
        private IEnumerator startDisableOutline(OutlineVmay outline)
        {
            ...
            if (Application.isPlaying)
            {
                ...
                foreach (Renderer r in rr)
                    if (!isExtrude(r.gameObject)) {
                        OutlineVmay outline = r.GetComponent<OutlineVmay>();
                        if (!outline) outline = r.gameObject.AddComponent<OutlineVmay>();
        ...
        protected virtual void setOutlineVisibility(bool value)
        {
            ...
            OutlineVmay[] items = GetComponentsInChildren<OutlineVmay>(true);
            foreach (OutlineVmay ouline in items)
